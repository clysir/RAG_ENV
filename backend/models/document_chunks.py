...
class DocumentChunk(Base):
    ...
    knowledge_base_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("knowledge_base.id"), nullable=False
    )
    chunk_index: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="chunk在当前文档中的顺序"
    )
    content: Mapped[str] = mapped_column(Text, nullable=False, comment="chunk文本内容")
    metadata_json: Mapped[dict[str, Any] | None] = mapped_column(
        JSON, nullable=False, comment="chunk元数据， 比如页码、soucre、doc_id、kb_id等"
    )
    content_hash: Mapped[str] = mapped_column(
        String(64), nullable=False, comment="chunk内容hash"
    )
    vector_id: Mapped[str] = mapped_column(
        String(128), nullable=False, unique=True, comment="向量库中的ID"
    )

    vector_store: Mapped[str] = mapped_column(
        String(50), nullable=False, default="chroma", comment="向量库存储类型"
    )
