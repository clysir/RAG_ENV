...
from backend.schemas.knowledge import KnowledgeResponse,KnowledgeBaseCreate,KnowledgeDetailResponse,KnowledgeBaseUpdate
from sqlalchemy.ext.asyncio import AsyncSession
from backend.app.knowledge_base.service import _create_kb,_list_kb,_get_kb_detail,_update_kb,_delete_kb
...
async def create_kb(
    ...
    try:
        return await _create_kb(kb_in, cur_user.id, db)
...
async def list_kb(
    ...
    try:
        # 这里后续再进行分页等功能的实现 先占位
        return await _list_kb(cur_user.id, db, skip, limit)
    except Exception as e:
        raise e
...
async def get_kb_detail(
    ...
    try:
        return await _get_kb_detail(kb_id, cur_user.id, db)
    except Exception as e:
        raise e
...
async def update_kb(
    ...
    try:
        return await _update_kb(kb_id, kb_in, cur_user.id, db)
    except Exception as e:
        raise e
...
async def delete_kb(
    ...
    try:
        return await _delete_kb(kb_id, cur_user.id, db)
    except Exception as e:
        raise e