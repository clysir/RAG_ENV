...
async def create_kb(
    kb_in: KnowledgeBaseCreate, user_id: int, db: AsyncSession
) -> Knowledge:
    """
    内部函数：创建知识库
    """
    kb = Knowledge(user_id=user_id, name=kb_in.name, description=kb_in.description)
    db.add(kb)
    await db.commit()
    await db.refresh(kb)
    return kb
...
async def list_kb(
    user_id: int, db: AsyncSession, skip: int = 0, limit: int = 100
) -> list[Knowledge]:
    """
    内部函数：获取知识库列表
    """
...
async def update_kb(
    kb_id: int, kb_in: KnowledgeBaseUpdate, user_id: int, db: AsyncSession
) -> Knowledge | None:
