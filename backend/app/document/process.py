...
def load_file_to_documents(
    temp_path: str, file_name: str
) -> list[LangChainDocument]:  # 这里的filename 也是数据库里面的
    """
...
def build_vector_id(kb_id: int, doc_id: int, chunk_index: int, content: str) -> str:
    """
    生成vector_id
...
def build_chunks(
    doc: DocumentToProcess, chunks: list[LangChainDocument]
    ...
    for index, chunk in enumerate(chunks):
        ...
        langchain_doc = LangChainDocument(page_content=content, metadata=metadata_json)
...
def parse_and_chunk_from_minio(
    ...
    try:
        temp_path = download_minio_object_to_temp(
            file_path=doc.file_path, suffix=suffix
        )

        raw_documents = load_file_to_documents(
            temp_path=temp_path, file_name=doc.file_name
        )

        chunks = split_documents(
            documents=raw_documents, chunk_size=chunk_size, chunk_overlap=chunk_overlap
        )

        built_chunks = build_chunks(doc=doc, chunks=chunks)
...
def add_chunks_to_vector_store(
    kb_id: int, chunks: list[ChunkBuildResult], batch_size: int = 1
) -> None:
    """
    测试版：分批写入 Chroma。
